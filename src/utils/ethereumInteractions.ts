...
import { stores, storesManagement } from './connections'

let provider = null

const getProvider = async (web3authProvider: any) => {
    provider = new ethers.providers.Web3Provider(web3authProvider)
    return provider
}

export const getRestaurants = async (web3authProvider: any) => {
    const storesManagementContract: any = await storesManagement(web3authProvider)
    const storesAddresses = await storesManagementContract.getStores()
    const storesData = []

    for (let i = 0; i < storesAddresses; i++) {
        const storeObject: any = {}
        const storesContract = await stores(web3authProvider, storesAddresses[i])
...
const createRestaurants = async (web3authProvider: any) => {
    const storesManagementContract: any = await storesManagement(web3authProvider)
    const storesAddresses = await storesManagementContract.getStores()
    const storesData = []
...
//         await campaignContract.addDonation(hashedName, hashedNumber, accounts[0], {
...
//         if (nftLink != '') {
//             const formatedIpfsLink = 'https://ipfs.io/ipfs/' + formatedLink.slice(7)
